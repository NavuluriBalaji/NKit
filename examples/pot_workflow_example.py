...
def create_mock_tools():
    ...
    def filter_records(records: dict, condition: str):
    ...
    def calculate_stats(records: dict):
        """Calculate statistics on records."""
        return {
    ...
    def generate_report(total_customers: int, statistics: dict, timestamp: str):
...
""",
    ...
    tools = {
        "load_csv": Tool("load_csv", load_csv, "Load CSV file and return records"),
        "filter_records": Tool("filter_records", filter_records, "Filter records based on condition"),
        "count_records": Tool("count_records", count_records, "Count number of records"),
        "calculate_stats": Tool("calculate_stats", calculate_stats, "Calculate statistics on records"),
        "generate_report": Tool("generate_report", generate_report, "Generate text report from statistics"),
    }
...
async def example_1_manual_execution():
    ...
    # Setup
    registry = ToolRegistry(include_builtin=False)  # Create empty registry
    tools = create_mock_tools()
    for name, tool in tools.items():
        registry.register(tool)  # Register tool object directly
...
def example_2_agent_pot_mode():
    ...
    print("\n💡 Usage:")
    print("   from NKit.agent import Agent")
